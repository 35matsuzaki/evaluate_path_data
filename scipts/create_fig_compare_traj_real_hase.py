...
#    target_dir = argvs[1]
    target_dir = "/home/sango/workspace/evaluate/compare_real/"
    ...
    for dir in sorted(os.listdir(target_dir)):
        ...
        end_index = np.argmin(np.abs(data[1:,X_ROW].astype(float)-16.5))
        print end_index
        if dir.split("_")[1] == "A":
            # ax1.plot(np.arange(0,data[:,X_ROW].shape[0],1),data[:,X_ROW], ".")
            #ax1.plot(data[1:end_index,X_ROW], data[1:end_index,Y_ROW], "k", label="without DWA")
            print data[-50,0].astype(float) - data[end_index,0].astype(float)
        if dir.split("_")[1] == "B":
            # ax2.plot(np.arange(0,data[:,X_ROW].shape[0],1),data[:,X_ROW], ".")
            #ax1.plot(data[1:end_index,X_ROW], data[1:end_index,Y_ROW], "r", label="min")
            print data[-50,0].astype(float) - data[end_index,0].astype(float)
        if dir.split("_")[1] == "C":
            # ax3.plot(np.arange(0,data[:,X_ROW].shape[0],1),data[:,X_ROW], ".")
            ax1.plot(data[1:end_index,X_ROW], data[1:end_index,Y_ROW], "k", label="max")
            print data[-50,0].astype(float) - data[end_index,0].astype(float)
        if dir.split("_")[1] == "D":
            # ax4.plot(np.arange(0,data[:,X_ROW].shape[0],1),data[:,X_ROW], ".")
            #ax1.plot(data[1:end_index,X_ROW], data[1:end_index,Y_ROW], "g", label="variable")
            print data[-50,0].astype(float) - data[end_index,0].astype(float)
    ...
    ax1.set_ylim(5,9)
    # plt.show()
    plt.savefig("compare_traj_real.png")
