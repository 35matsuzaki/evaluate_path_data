...
TIME_ROW = 0
X_ROW = 22 # 8:odom, 10:cmd
Y_ROW = 23 # 9:odom, 10:cmd
X_MAX_ = 7.0
X_MIN_ = -8.0
TIME_MAX = 15
TIME_MIN = 5
...
class PlotPotential:
    ...
    def setData(self, dir_name):
        for file in os.listdir(dir_name):
            ...
            #ロボットの軌跡ファイル以外に対して
            if file_path.split("/")[-1] == "_slash_cmd_state.csv":
    ...
    def run(self):
        ...
        return np.sum(np.abs(min_theta) < threth)*0.01

    def plotHist(self):
        # labels = ['without DWA', 'min', 'max', 'variable']
        # self.ax1.hist([self.A_hist_result,self.B_hist_result,self.C_hist_result,self.D_hist_result],color=['black','red', 'blue', 'green'],  bins=5,label=labels)
    ...
    def printResult(self):
        ...
        print
        print np.std(self.A_hist_result),np.std(self.B_hist_result),np.std(self.C_hist_result),np.std(self.D_hist_result)
