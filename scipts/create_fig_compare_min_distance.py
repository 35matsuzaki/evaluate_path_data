...
class PlotPotential:
    def __init__(self):
        ...
        self.var = 1
        self.fig = plt.figure()
        self.ax1 = self.fig.add_subplot(111) #左上
        self.actor_data = []
        self.robot_data = []
    ...
    def plotHist(self):
        ...
        self.ax1.hist([self.A_hist_result,self.B_hist_result,self.C_hist_result,self.D_hist_result],color=['black','red', 'blue', 'green'], range=(self.z_min, self.z_max), bins=5,label=labels)
        tmp = 160
        self.ax1.set_ylim(0,tmp)
        plt.legend()
        plt.savefig("compare_min_dist.eps")
        plt.show()
...
if __name__ == "__main__":
    args = sys.argv
    if len(args) != 1:
        print "usage: $python %s" % args[0]
        print "usage: arg[1] = tome result dir (i.e ./tome/all)"
        exit()
    # dir_name = args[1]
    dir_name = "/home/sango/workspace/evaluate/tome/all"
